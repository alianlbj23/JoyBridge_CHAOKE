...
class RosPublisher:
    def __init__(self, client, topics_config, arm_config=None, cmd_vel_config=None):
        ...
        self.arm_time_from_start_sec = 1
        if arm_config:
            ...
            self.arm_joint_names = arm_config.get('joint_names', [])
            self.arm_time_from_start_sec = arm_config.get('time_from_start_sec', 1)

    def send_arm_joints(self, joint_angles_deg):
        """發送機械手臂 JointTrajectory (輸入為度，轉換為弧度發布)。"""
        if self.arm_publisher:
                ...
                'points': [
                    {
                        ...
                        'effort': [],
                        'time_from_start': {
                            'sec': self.arm_time_from_start_sec,
                            'nanosec': 0
                        }
                    }
                ]
            ...
            print(
                f"[ARM] Published trajectory: joints={self.arm_joint_names}, "
                f"angles={joint_angles_deg}° -> {joint_angles_rad} rad"
            )
